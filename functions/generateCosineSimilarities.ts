...
export const generateCosineSimilarities = (props: FunctionProps): (number | string)[][] => {
    ...
    const comparisons = vectors.map((vector: any[]) => {
        ...
        const sumOfSquares = vectorWithoutStageId.map((number: number) => number * number)
            .reduce((runningTotal, currentNumber) => runningTotal + currentNumber)
        const magnitude = Math.sqrt(sumOfSquares)

        const cosineSimilarities = vectors.map((secondVector: any[]) => { // TODO: I could map over a clone of the original vectors, and then slice off the last vector each iteration to improve performance (since you only need half half of the table)
            
            const secondVectorWithoutStageId = secondVector.slice(1);
            const secondSumOfSquares = secondVectorWithoutStageId.map((number: number) => number * number)
                .reduce((runningTotal, currentNumber) => runningTotal + currentNumber);
            const secondMagnitude = Math.sqrt(secondSumOfSquares);

            const dotProduct = getDotProduct(vectorWithoutStageId, secondVectorWithoutStageId);
